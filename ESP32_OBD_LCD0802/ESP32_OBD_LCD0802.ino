...
// ---------- LCD 0802 parallel 4-bit ----------
// RS=19, E=23, D4=18, D5=17, D6=16, D7=15
...
void lcdShow(bool connected) {
  ...
  if (hasData) {
    char buf[9];
    snprintf(buf, sizeof(buf), "I:%5.1fA", -lastA);
    lcd.print(buf);
  } else {
    lcd.print("I:--.-A ");
  }
  ...
  if (lastA > 0.2f)      lcd.print("DISCHG  ");
  else if (lastA < -0.2f) lcd.print("CHG     ");
...
bool connectOBD() {
  ...
  while (true) {
    ...
    lcd.setCursor(0, 1);
    char buf[9];
    snprintf(buf, sizeof(buf), "Try: %d", attempt);
    lcd.print(buf);
